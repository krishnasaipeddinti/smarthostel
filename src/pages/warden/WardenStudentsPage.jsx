...
const WardenStudentsPage = () => {
  ...
  const handleAssignRoom = async (studentId) => {
    try {
      ...
      setStudents((prev) =>
        prev.map((item) => (item.id === studentId ? { ...item, ...updatedStudent } : item))
      );
        ...
        {students.map((student) => (
          <div key={student.id} className="glass rounded-3xl p-5 shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <button className="btn-secondary" onClick={() => handleEdit(student)}>
                Edit
              </button>
            </div>
            ...
            {editingId === student.id ? (
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                ...
                <input className="input" value={editForm.course} onChange={(e) => setEditForm({ ...editForm, course: e.target.value })} placeholder="Course" />
                <input className="input" value={editForm.year} onChange={(e) => setEditForm({ ...editForm, year: e.target.value })} placeholder="Year" />
                <input className="input md:col-span-2" value={editForm.parentContact} onChange={(e) => setEditForm({ ...editForm, parentContact: e.target.value })} placeholder="Parent Contact" />
              ...
              <div className="mt-4 grid gap-3 text-sm text-slate-300 md:grid-cols-2">
                ...
                <p><span className="font-semibold text-white">Parent Contact:</span> {student.parentContact || "-"}</p>
                <p><span className="font-semibold text-white">Room:</span> {student.room || "Not Allotted"}</p>
                <p><span className="font-semibold text-white">Hostel Block:</span> {student.hostelBlock || "-"}</p>
              </div>
            )}
            ...
            <div className="mt-5 flex flex-col gap-3 md:flex-row md:items-center">
              <select
                ...
                onChange={(e) =>
                  setAssignMap((prev) => ({ ...prev, [student.id]: e.target.value }))
                }
              >
                <option value="">Select room</option>
                {rooms.map((room) => (
                  <option key={room.id} value={room.id}>
                    {room.roomNo} • {room.roomType} • {room.sharing} Share • {room.occupied}/{room.capacity}
                  </option>
                ))}
              </select>

              <button className="btn-primary" onClick={() => handleAssignRoom(student.id)}>
                Assign Room
              </button>
            </div>
          </div>
